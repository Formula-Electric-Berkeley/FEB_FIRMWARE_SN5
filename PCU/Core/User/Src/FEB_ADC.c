...
ADC_StatusTypeDef FEB_ADC_GetBrakeData(Brake_DataTypeDef *brake_data)
{
  ...
  brake_data->pressure1_bar = FEB_ADC_MapRange(
      pressure1_voltage, brake_pressure1_calibration.min_voltage, brake_pressure1_calibration.max_voltage,
      brake_pressure1_calibration.min_physical, brake_pressure1_calibration.max_physical);
  brake_data->pressure2_bar = FEB_ADC_MapRange(
      pressure2_voltage, brake_pressure2_calibration.min_voltage, brake_pressure2_calibration.max_voltage,
      brake_pressure2_calibration.min_physical, brake_pressure2_calibration.max_physical);
  ...
  brake_data->brake_position = FEB_ADC_MapRange(avg_pressure, 0.0f, BRAKE_PRESSURE_MAX_PHYSICAL_BAR, 0.0f, 100.0f);
...
ADC_StatusTypeDef FEB_ADC_GetDiagnostics(char *buffer, size_t size)
{
  ...
  printf(buffer, size,
         ...
         apps_data.position1, apps_data.position2, apps_data.plausible ? "Yes" : "No", brake_data.pressure1_bar,
         brake_data.pressure2_bar, brake_data.brake_pressed ? "Yes" : "No", active_faults, adc_runtime.error_count);
...
static uint16_t GetAveragedADCValue(ADC_HandleTypeDef *hadc, uint32_t channel, uint8_t samples)
{
  ...
  else if (hadc == &hadc3)
  {
    ...
    if (channel == ADC_CHANNEL_10)
      ...
      channel_idx = ADC3_CH12_ACC_PEDAL2_IDX;
    else if (channel == ADC_CHANNEL_13)
      channel_idx = ADC3_CH13_ACC_PEDAL1_IDX;
    else
      return 0;
  }
  else
