                                     ...
                                     FEB_CAN_RX_Callback_t callback)
{
  ...
  LOG_I(TAG_CAN, "Registered RX callback: CAN%d, ID=0x%03lX (total: %lu)", instance + 1, can_id,
        feb_can_rx_registered_count);
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
  ...
  if (HAL_CAN_GetRxMessage(hcan, CAN_RX_FIFO0, &feb_can_rx_header[instance], feb_can_rx_data[instance]) == HAL_OK)
  {
    ...
    LOG_D(TAG_CAN, "RX CAN%d: ID=0x%03lX, Len=%lu, Data: %02X %02X %02X %02X %02X %02X %02X %02X", instance + 1, can_id,
          feb_can_rx_header[instance].DLC, feb_can_rx_data[instance][0], feb_can_rx_data[instance][1],
          feb_can_rx_data[instance][2], feb_can_rx_data[instance][3], feb_can_rx_data[instance][4],
          feb_can_rx_data[instance][5], feb_can_rx_data[instance][6], feb_can_rx_data[instance][7]);
