 ...
 */
#define LOG_FORMAT(color, level, tag, fmt) \
    color "[%lu] %s " level ": " fmt ANSI_COLOR_RESET "\r\n"
...
#define LOG_I(tag, fmt, ...) \
    do { \
        if (FEB_DEBUG_GLOBAL_LEVEL >= DEBUG_LEVEL_INFO) { \
            printf(LOG_FORMAT(ANSI_COLOR_CYAN, "INFO", tag, fmt), \
...
#define LOG_W(tag, fmt, ...) \
    do { \
        if (FEB_DEBUG_GLOBAL_LEVEL >= DEBUG_LEVEL_WARN) { \
            printf(LOG_FORMAT_LOC(ANSI_COLOR_YELLOW ANSI_BOLD, "WARN", tag, fmt, __FILE__, __LINE__), \
                   HAL_GetTick(), tag, __FILE__, __LINE__, ##__VA_ARGS__); \
        } \
    } while(0)
...
#define LOG_E(tag, fmt, ...) \
    do { \
        if (FEB_DEBUG_GLOBAL_LEVEL >= DEBUG_LEVEL_ERROR) { \
            printf(LOG_FORMAT_LOC(ANSI_COLOR_RED ANSI_BOLD, "ERROR", tag, fmt, __FILE__, __LINE__), \
                   HAL_GetTick(), tag, __FILE__, __LINE__, ##__VA_ARGS__); \
        } \
    } while(0)
...
#define LOG_D(tag, fmt, ...) \
    do { \
        if (FEB_DEBUG_GLOBAL_LEVEL >= DEBUG_LEVEL_DEBUG) { \
            printf(LOG_FORMAT(ANSI_COLOR_MAGENTA, "DEBUG", tag, fmt), \
                   HAL_GetTick(), tag, ##__VA_ARGS__); \
        } \
    } while(0)
