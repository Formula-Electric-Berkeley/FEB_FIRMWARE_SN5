...
void TPS2482_Init(I2C_HandleTypeDef *hi2c, uint8_t *addresses, TPS2482_Configuration *configurations, uint16_t *ids, bool *res, uint8_t messageCount) {
	...
	printf("[TPS2482] Init start, devices=%u\r\n", (unsigned int)messageCount);
	...
	for ( uint8_t i = 0; i < messageCount; i++ ) {
		...
		printf("[TPS2482] addr=0x%02X id=0x%04X ok=%u\r\n",
				(unsigned int)addresses[i], (unsigned int)ids[i], (unsigned int)res[i]);
		printf("[TPS2482] cfg=0x%04X rd=0x%04X cal=0x%04X rd=0x%04X\r\n",
				(unsigned int)configs[i], (unsigned int)configs_res[i],
				(unsigned int)cals[i], (unsigned int)cals_res[i]);
		printf("[TPS2482] mask=0x%04X rd=0x%04X alert=0x%04X rd=0x%04X\r\n",
				(unsigned int)masks[i], (unsigned int)masks_res[i],
				(unsigned int)alert_limits[i], (unsigned int)alert_limits_res[i]);
	}
}
...
void TPS2482_Get_Register(I2C_HandleTypeDef *hi2c, uint8_t *addresses, uint8_t reg, uint16_t *results, uint8_t messageCount) {
	...
	for ( uint8_t i = 0; i < messageCount; i++ ) {
		HAL_StatusTypeDef status = FEB_I2C_Mem_Read(hi2c, addresses[i] << 1, reg, I2C_MEMADD_SIZE_8BIT, &res[2*i], sizeof(*results), HAL_MAX_DELAY);
		if ( status != HAL_OK ) {
			printf("[TPS2482] I2C read failed addr=0x%02X reg=0x%02X status=%d\r\n",
					(unsigned int)addresses[i], (unsigned int)reg, (int)status);
		} else {
...
void TPS2482_Write_Register(I2C_HandleTypeDef *hi2c, uint8_t *addresses, uint8_t reg, uint16_t *transmit, uint8_t messageCount) {
	...
	for ( uint8_t i = 0; i < messageCount; i++ ) {
		...
		HAL_StatusTypeDef status = FEB_I2C_Mem_Write(hi2c, addresses[i] << 1, reg, I2C_MEMADD_SIZE_8BIT, &trans[2 * i], sizeof(uint16_t), HAL_MAX_DELAY);
		if ( status != HAL_OK ) {
			printf("[TPS2482] I2C write failed addr=0x%02X reg=0x%02X status=%d\r\n",
					(unsigned int)addresses[i], (unsigned int)reg, (int)status);
		}
	}
}
...
//	uint8_t buf[12];
//	buf[0] = REG;
//	HAL_StatusTypeDef ret = FEB_I2C_Master_Transmit(hi2c, DEV_ADDR, buf, 1, 100);
//	if (ret != HAL_OK) {
//		HAL_GPIO_WritePin(EN, EN_NUM, GPIO_PIN_RESET); // pull EN low
//	} else {
//		ret = FEB_I2C_Master_Receive(hi2c, DEV_ADDR, buf, 2, 100);
//		if (ret != HAL_OK) {
//			HAL_GPIO_WritePin(EN, EN_NUM, GPIO_PIN_RESET); // pull EN low
//		} else {
