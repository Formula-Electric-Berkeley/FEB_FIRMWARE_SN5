 ...
 * Getters
 *
 * Plain reads, no locking: aligned 32-bit loads are atomic on Cortex-M4, and
 * these are read from the 1 ms state-machine task, which must never block.
 * Callers needing multi-field consistency use BMS_Pack_Snapshot() (seqlock).
 *============================================================================*/
...
{
  /* Request-only: reachable from the 1 ms SM task (fault entry), which must
   * never block. The acquisition task performs the actual bus writes. */
...
{
  /* BMS_ErrorSource_t bits are defined bit-compatible with the
   * ADBMS_FAULT_FLAG_* values the SM consumes (see FEB_ADBMS_App.h). */
  return g_bms_pack.active_error_mask;
}

uint32_t FEB_ADBMS_Get_Last_Update_Tick(void)
{
  return g_bms_pack.last_update_tick_ms;
}
...
{
  /* NAN until the first valid scan: `!(NAN < limit)` is true, so thermal
   * gates fail closed with no telemetry — same contract as the old driver. */
  if (!g_bms_pack.temp_valid)
    return NAN;
  return g_bms_pack.pack_max_temp_C;
}
...
{
  return BMS_App_GetCellVoltage(bank, (uint8_t)(cell / BMS_CELLS_PER_IC), (uint8_t)(cell % BMS_CELLS_PER_IC));
}

float FEB_ADBMS_GET_Cell_Voltage_S(uint8_t bank, uint16_t cell)
{
  return BMS_App_GetCellVoltageS(bank, (uint8_t)(cell / BMS_CELLS_PER_IC), (uint8_t)(cell % BMS_CELLS_PER_IC));
}

float FEB_ADBMS_GET_ACC_MIN_Temp(void)
{
  return BMS_App_GetMinTemp();
}
float FEB_ADBMS_GET_ACC_MAX_Temp(void)
{
  return BMS_App_GetMaxTemp();
}
float FEB_ADBMS_GET_ACC_AVG_Temp(void)
{
  return BMS_App_GetAvgTemp();
}

bool FEB_Cell_Balance_Complete(void)
{
  return g_bms_pack.voltage_valid && !BMS_App_IsBalancingNeeded();
}
