...
{
  if (refon)
    ic[nIC].configa.tx_data[0] = ic[nIC].configa.tx_data[0] | REFON_MASK;
  else
    ic[nIC].configa.tx_data[0] = ic[nIC].configa.tx_data[0] & REFON_CLEAR_MASK;
}
...
uint32_t ADBMS6830B_pollAdc()
{
  ...
  while ((counter < ADC_POLL_TIMEOUT) && (finished == 0))
  {
    current_time = FEB_spi_read_byte(BYTE_MASK);
    if (current_time > 0)
    {
      finished = 1;
    }
    ...
    {
      counter = counter + ADC_POLL_INCREMENT;
    }
  }
...
)
{
  ...
  for (int REGGRP = 0; REGGRP < 6; REGGRP++)
  {
    ...
    for (int icn = 0; icn < total_ic; icn++)
    {
      ...
      // rdcv writes pec_match[REGGRP] unconditionally; rdsv runs after
      // (FEB_ADBMS6830B.c:146-147) and ORs in its result so the flag is
...
)
{
  ...
  for (int REGGRP = 0; REGGRP < 6; REGGRP++)
  {
    ...
    for (int icn = 0; icn < total_ic; icn++)
    {
      ...
      // Per-IC PEC: byte 6 = {CC[5:0], PEC[9:8]}, byte 7 = PEC[7:0].
      // Worst-of-two with rdcv: rdcv runs first (FEB_ADBMS6830B.c:146-147)
      // and writes pec_match[REGGRP] unconditionally; we OR in rdsv's result
      // so a rdcv PEC fail is not silently cleared by a rdsv pass. The
...
)
{
  ...
  if (cell_data == NULL)
  {
    ...
    {
      ic[bank].configa.rx_pec_match = -1;
    }
    return;
  }
...
)
{
  ...
  {
    c_ic = current_ic;
    for (uint8_t data = 0; data < REG_DATA_SIZE; data++)
    {
...
)
{
  ...
  if (cell_data == NULL)
  {
    ...
    for (int bank = 0; bank < total_ic; bank++)
    {
      ic[bank].configb.rx_pec_match = -1;
    }
    return;
  }
...
{
  uint8_t flagData[MAX_WRITE_BUFFER_SIZE];
  uint8_t write_count = 0;
  for (uint8_t current_ic = 0; current_ic < total_ic; current_ic++)
  {
    for (uint8_t data = 0; data < REG_DATA_SIZE; data++)
    {
      flagData[write_count] = BYTE_MASK;
      write_count++;
    }
  }
