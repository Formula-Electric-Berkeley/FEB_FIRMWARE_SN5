                  ...
                  bool *res, uint8_t messageCount)
{
  ...
  for (uint8_t i = 0; i < messageCount; i++)
  {
    ...
    uint16_t masks_flagless_mask = TPS2482_MASK_SOL | TPS2482_MASK_SUL | TPS2482_MASK_BOL | TPS2482_MASK_BUL |
                                   TPS2482_MASK_CNVR | TPS2482_MASK_POL;
    res[i] &= masks[i] == (masks_res[i] & masks_flagless_mask);
    res[i] &= alert_limits[i] == alert_limits_res[i];
  }
}

void TPS2482_Get_Register(I2C_HandleTypeDef *hi2c, uint8_t *addresses, uint8_t reg, uint16_t *results,
                          uint8_t messageCount)
{
  ...
  for (uint8_t i = 0; i < messageCount; i++)
  {
    if (FEB_I2C_Mem_Read(hi2c, addresses[i] << 1, reg, I2C_MEMADD_SIZE_8BIT, &res[2 * i], sizeof(*results),
...
void TPS2482_Write_Register(I2C_HandleTypeDef *hi2c, uint8_t *addresses, uint8_t reg, uint16_t *transmit,
                            uint8_t messageCount)
{
  ...
  for (uint8_t i = 0; i < messageCount; i++)
  {
    ...
    if (FEB_I2C_Mem_Write(hi2c, addresses[i] << 1, reg, I2C_MEMADD_SIZE_8BIT, &trans[2 * i], sizeof(uint16_t),
                          HAL_MAX_DELAY) != HAL_OK)
    {
      // Todo failure state
    }
...
// void FEB_TPS2482_shutdownIfError(I2C_HandleTypeDef * hi2c, uint8_t DEV_ADDR, GPIO_TypeDef * EN, uint16_t EN_NUM,
// GPIO_TypeDef * AL, uint16_t AL_NUM, 		GPIO_TypeDef * PG, uint16_t PG_NUM, float VMAX, float VMIN, float IMAX, float
// IMIN, float PMAX, float PMIN) {
//
//	// pull EN low if
...
// void FEB_TPS2482_pullLowIfOutOfBounds(I2C_HandleTypeDef * hi2c, uint8_t DEV_ADDR, GPIO_TypeDef * EN, uint16_t EN_NUM,
// float MAX, float MIN, 		uint8_t REG) { 	uint8_t buf[12]; 	buf[0] = REG; 	HAL_StatusTypeDef ret =
// FEB_I2C_Master_Transmit(hi2c, DEV_ADDR, buf, 1, 100); 	if (ret != HAL_OK) { 		HAL_GPIO_WritePin(EN, EN_NUM,
// GPIO_PIN_RESET); // pull EN low 	} else { 		ret = FEB_I2C_Master_Receive(hi2c, DEV_ADDR, buf, 2, 100); 		if (ret !=
// HAL_OK) { 			HAL_GPIO_WritePin(EN, EN_NUM, GPIO_PIN_RESET); // pull EN low 		} else { 			int val =
//((int16_t)buf[0] << 4) | (buf[1] >> 4); // combine the 2 bytes 			val = val - 1; 			val |= 0xF000; // subtract 1
// and take complement 			float parsed = val * 0.0000025;// convert to decimal and multiply by 2.5uV 			if (parsed >
// MAX || parsed < MIN) { 				HAL_GPIO_WritePin(EN, EN_NUM, GPIO_PIN_RESET);
//			}
//		}
//	}
// }
