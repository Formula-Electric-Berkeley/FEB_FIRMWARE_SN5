...
//GPIO_PIN_RESET); // pull EN low 	} else { 		ret = FEB_I2C_Master_Receive(hi2c, DEV_ADDR, buf, 2, 100); 		if (ret != HAL_OK)
//{ 			HAL_GPIO_WritePin(EN, EN_NUM, GPIO_PIN_RESET); // pull EN low 		} else { 			int val = ((int16_t)buf[0] << 4) | (buf[1] >>
//4); // combine the 2 bytes 			val = val - 1; 			val |= 0xF000; // subtract 1 and take complement 			float parsed = val *
//0.0000025;// convert to decimal and multiply by 2.5uV 			if (parsed > MAX || parsed < MIN) { 				HAL_GPIO_WritePin(EN,
//EN_NUM, GPIO_PIN_RESET);
//			}
//		}
//	}
// }
