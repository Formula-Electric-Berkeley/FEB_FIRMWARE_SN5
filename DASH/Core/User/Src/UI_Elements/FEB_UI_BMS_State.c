...
static lv_obj_t *ui_BMS_State;

void FEB_UI_Update_BMS_State()
{
  BMS_State_t state = FEB_CAN_BMS_GetLastState();
  lv_label_set_text(ui_BMS_State, to_BMS_state_string(state));
}
...
{
  ui_BMS_State = lv_label_create(ui_Screen);
  lv_obj_align(ui_BMS_State, LV_ALIGN_BOTTOM_RIGHT, -15, -15);
  lv_label_set_text(ui_BMS_State, "---");
  lv_obj_set_style_text_font(ui_BMS_State, &lv_font_montserrat_40, 0);
  lv_obj_set_style_text_color(ui_BMS_State, lv_color_hex(0xFFFFFF), 0);
}
