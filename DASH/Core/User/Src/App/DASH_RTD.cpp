...
#define RTD_INPUT_FRESHNESS_MS 1000 /* PCU brake only; BMS uses the DBC cycle time */
...
void FEB_State_Update_RTD(void)
{
  ...
  BMS_State_t bms_state = static_cast<BMS_State_t>(fc::rx<fm::BmsState>.v().bms_state);
  ...
  uint16_t brake_pressure = fc::rx<fm::Brake>.v().brake_position;

  bool inputs_fresh = fc::rx<fm::Brake>.fresh() && fc::rx<fm::BmsState>.fresh();
