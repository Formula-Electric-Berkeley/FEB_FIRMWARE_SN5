...
{
  /* NOTE: This does NOT use the combined RDCSALL opcode. It performs two
   * separate transactions (CVALL then SVALL), so the C and S snapshots are
   * not guaranteed to be atomic with respect to each other. */
...
{
  /* NOTE: This does NOT use the combined RDACSALL opcode. It performs two
   * separate transactions (ACALL then SVALL); not an atomic snapshot. */
...
{
  /* NOTE: This does NOT use the combined RDASALL opcode. It performs two
   * separate transactions (AUXA..D then STATA..E); not an atomic snapshot. */
...
<<<<<<< HEAD
  ...
  if (voltage_mV < 1500)
    return 0;
  uint32_t code = ((uint32_t)(voltage_mV - 1500) * 10) / 24;
  if (code > 0x0FFF)
    code = 0x0FFF;
