...
static HAL_StatusTypeDef feb_tps_read_reg(I2C_HandleTypeDef *hi2c, uint8_t i2c_addr, uint8_t reg, uint16_t *value)
{
  uint8_t buf[2];
  HAL_StatusTypeDef status =
      HAL_I2C_Mem_Read(hi2c, (uint16_t)(i2c_addr << 1), reg, I2C_MEMADD_SIZE_8BIT, buf, 2, FEB_TPS_I2C_TIMEOUT_MS);
  if (status == HAL_OK)
  {
...
static HAL_StatusTypeDef feb_tps_write_reg(I2C_HandleTypeDef *hi2c, uint8_t i2c_addr, uint8_t reg, uint16_t value)
{
  uint8_t buf[2] = {(uint8_t)(value >> 8), (uint8_t)(value & 0xFF)};
  return HAL_I2C_Mem_Write(hi2c, (uint16_t)(i2c_addr << 1), reg, I2C_MEMADD_SIZE_8BIT, buf, 2, FEB_TPS_I2C_TIMEOUT_MS);
}
...
FEB_TPS_Status_t FEB_TPS_Init(const FEB_TPS_LibConfig_t *config)
{
  ...
  if (config != NULL)
  {
    ...
    feb_tps_ctx.log_level = (config->log_level != 0) ? config->log_level : FEB_TPS_LOG_INFO;
  }
  else
  {
...
FEB_TPS_Status_t FEB_TPS_DeviceRegister(const FEB_TPS_DeviceConfig_t *config, FEB_TPS_Handle_t *handle)
{
  if (!feb_tps_ctx.initialized)
  {
  ...
  const uint16_t config_val = (config->config_reg != 0) ? config->config_reg : FEB_TPS_CONFIG_REG_DEFAULT;
  ...
  hal_status = feb_tps_write_reg(dev->hi2c, dev->i2c_addr, FEB_TPS_REG_CONFIG, config_val);
  if (hal_status != HAL_OK)
  {
    TPS_LOG_E("CONFIG write failed for %s @ 0x%02X", dev->name ? dev->name : "?", dev->i2c_addr);
    status = FEB_TPS_ERR_I2C;
    goto cleanup;
  }

  hal_status = feb_tps_write_reg(dev->hi2c, dev->i2c_addr, FEB_TPS_REG_CAL, dev->cal_reg);
  if (hal_status != HAL_OK)
  {
    TPS_LOG_E("CAL write failed for %s @ 0x%02X", dev->name ? dev->name : "?", dev->i2c_addr);
    status = FEB_TPS_ERR_I2C;
    goto cleanup;
  }
  ...
  {
    hal_status = feb_tps_write_reg(dev->hi2c, dev->i2c_addr, FEB_TPS_REG_MASK, config->mask_reg);
    if (hal_status != HAL_OK)
    {
      status = FEB_TPS_ERR_I2C;
      goto cleanup;
  ...
  {
    hal_status = feb_tps_write_reg(dev->hi2c, dev->i2c_addr, FEB_TPS_REG_ALERT_LIM, config->alert_limit);
    if (hal_status != HAL_OK)
    {
      status = FEB_TPS_ERR_I2C;
      goto cleanup;
  ...
  hal_status = feb_tps_read_reg(dev->hi2c, dev->i2c_addr, FEB_TPS_REG_CONFIG, &rb_config);
  if (hal_status != HAL_OK)
  {
  ...
  hal_status = feb_tps_read_reg(dev->hi2c, dev->i2c_addr, FEB_TPS_REG_CAL, &rb_cal);
  if (hal_status != HAL_OK)
  {
  ...
  {
    TPS_LOG_E("CONFIG mismatch %s: wrote 0x%04X read 0x%04X", dev->name ? dev->name : "?", config_val, rb_config);
    status = FEB_TPS_ERR_CONFIG_MISMATCH;
    goto cleanup;
  }
  if (rb_cal != dev->cal_reg)
  {
    TPS_LOG_E("CAL mismatch %s: wrote 0x%04X read 0x%04X", dev->name ? dev->name : "?", dev->cal_reg, rb_cal);
    status = FEB_TPS_ERR_CONFIG_MISMATCH;
    goto cleanup;
  }
  ...
  *handle = dev;
  ...
  TPS_LOG_I("Registered '%s' at 0x%02X (cal=0x%04X)", dev->name ? dev->name : "?", dev->i2c_addr, dev->cal_reg);
...
FEB_TPS_Status_t FEB_TPS_Poll(FEB_TPS_Handle_t handle, FEB_TPS_Measurement_t *measurement)
{
  if (!feb_tps_ctx.initialized)
  {
    return FEB_TPS_ERR_NOT_INIT;
  ...
  hal_status = feb_tps_read_reg(dev->hi2c, dev->i2c_addr, FEB_TPS_REG_BUS_VOLT, &raw);
  if (hal_status != HAL_OK)
  {
    status = FEB_TPS_ERR_I2C;
    goto cleanup;
  ...
  hal_status = feb_tps_read_reg(dev->hi2c, dev->i2c_addr, FEB_TPS_REG_CURRENT, &raw);
  if (hal_status != HAL_OK)
  {
    status = FEB_TPS_ERR_I2C;
    goto cleanup;
  ...
  hal_status = feb_tps_read_reg(dev->hi2c, dev->i2c_addr, FEB_TPS_REG_SHUNT_VOLT, &raw);
  if (hal_status != HAL_OK)
  {
    status = FEB_TPS_ERR_I2C;
    goto cleanup;
  ...
  measurement->shunt_voltage_mv = (float)measurement->shunt_voltage_raw * FEB_TPS_CONV_VSHUNT_MV_PER_LSB;

  hal_status = feb_tps_read_reg(dev->hi2c, dev->i2c_addr, FEB_TPS_REG_POWER, &raw);
  if (hal_status != HAL_OK)
  {
    status = FEB_TPS_ERR_I2C;
    goto cleanup;
...
FEB_TPS_Status_t FEB_TPS_PollScaled(FEB_TPS_Handle_t handle, FEB_TPS_MeasurementScaled_t *scaled)
{
  ...
  {
    scaled->bus_voltage_mv = (meas.bus_voltage_v >= 0.0f) ? (uint32_t)(meas.bus_voltage_v * 1000.0f) : 0;
    scaled->current_ma = (int32_t)(meas.current_a * 1000.0f);
    scaled->shunt_voltage_uv = (int32_t)(meas.shunt_voltage_mv * 1000.0f);
    scaled->power_mw = (meas.power_w >= 0.0f) ? (uint32_t)(meas.power_w * 1000.0f) : 0;
  }
  return status;
}

FEB_TPS_Status_t FEB_TPS_PollRaw(FEB_TPS_Handle_t handle, uint16_t *bus_v_raw, int16_t *current_raw,
                                 int16_t *shunt_v_raw)
{
  if (!feb_tps_ctx.initialized)
  {
...
FEB_TPS_Status_t FEB_TPS_Enable(FEB_TPS_Handle_t handle, bool enable)
{
  ...
  HAL_GPIO_WritePin((GPIO_TypeDef *)dev->en_gpio_port, dev->en_gpio_pin, enable ? GPIO_PIN_SET : GPIO_PIN_RESET);
  return FEB_TPS_OK;
}
...
FEB_TPS_Status_t FEB_TPS_ReadPowerGood(FEB_TPS_Handle_t handle, bool *pg_state)
{
  ...
  GPIO_PinState s = HAL_GPIO_ReadPin((GPIO_TypeDef *)dev->pg_gpio_port, dev->pg_gpio_pin);
...
FEB_TPS_Status_t FEB_TPS_ReadID(FEB_TPS_Handle_t handle, uint16_t *id)
{
  ...
  FEB_TPS_MUTEX_LOCK(feb_tps_ctx.i2c_mutex);
  HAL_StatusTypeDef hal_status = feb_tps_read_reg(dev->hi2c, dev->i2c_addr, FEB_TPS_REG_ID, id);
  FEB_TPS_MUTEX_UNLOCK(feb_tps_ctx.i2c_mutex);
...
const char *FEB_TPS_StatusToString(FEB_TPS_Status_t status)
{
  ...
  case FEB_TPS_OK:
    return "OK";
  case FEB_TPS_ERR_INVALID_ARG:
    return "Invalid argument";
  case FEB_TPS_ERR_I2C:
    return "I2C error";
  case FEB_TPS_ERR_NOT_INIT:
    return "Not initialized";
  case FEB_TPS_ERR_CONFIG_MISMATCH:
    return "Config mismatch";
  case FEB_TPS_ERR_MAX_DEVICES:
    return "Max devices exceeded";
  default:
    return "Unknown";
  }
}
