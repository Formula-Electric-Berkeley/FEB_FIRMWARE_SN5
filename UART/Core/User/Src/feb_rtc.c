...
static bool acquire_mutex(void)
{
  if (rtcMutexHandle == NULL)
    return false;
  return osMutexAcquire(rtcMutexHandle, RTC_MUTEX_TIMEOUT_MS) == osOK;
}
...
/* Internal helper - reads RTC without mutex (caller must hold mutex) */
static FEB_RTC_Status_t get_datetime_internal(FEB_RTC_DateTime_t *dt)
{
...
FEB_RTC_Status_t FEB_RTC_GetDateTime(FEB_RTC_DateTime_t *dt)
{
  ...
  FEB_RTC_Status_t result = get_datetime_internal(dt);
...
FEB_RTC_Status_t FEB_RTC_GetTime(uint8_t *hours, uint8_t *minutes, uint8_t *seconds)
{
  ...
  FEB_RTC_Status_t status = get_datetime_internal(&dt);
